            ...
            async def wrapper(websocket: WebSocket | ClientConnection, ins_open: InsOpen, *args, **kwargs):
                ...
                if isinstance(result, dict):
                    ...
                    if type(websocket) == WebSocket:
                        ...
                        # 若响应目标是本地节点，直接注入 ResFutures()
                        if ins_open_send.receiver == NodeMeta()["bid"]:
                            ResFutures().res_ins(ins_open_send)
                        else:
    ...
    def cert(self, routing_key: str):
        ...
        def decorator(func: Callable):
            ...
            async def wrapper(node_model, ins_cert, *args, **kwargs):
                ...
                # ✅ 统一包装响应
                if isinstance(result, dict):
                    ins_cert_res = InsCertFactory().create(
                    ...
                    # 若响应目标是本地节点，直接注入 ResFutures()
                    if ins_cert_res.receiver == NodeMeta()["bid"]:
                        ResFutures().res_ins(ins_cert_res)
                    else:
