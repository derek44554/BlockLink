...
async def connection_node(websocket):
    # --------------- 获取对方证书
    ...
    await websocket.send(get_signature_ins_model.text)  # 发送消息
    challenge_response = await websocket.recv()  # 接收服务器返回的消息

    # 包含模型的证书的指令模型
    signature_ins_model = ins_open_factory.fro_text(challenge_response)
    # 创建签名对象
    ...
    await websocket.send(challenge_send_ins_model.text)  # 发送消息
    ...
    await websocket.send(ok_send_ins_model.text)  # 发送消息
    # --------------- 节点创建
    node_manager = NodeManager()
    node_model = NodeModel(bid=signature_model.owner, websocket=websocket, signature_model=signature_model, encryption_key=encryption_key)
