...
async def forward_to_node(websocket, data, sender, receiver, int_bid):
    ...
    if NodeMeta().node.get("bridge") != True:
        ins_cert_send = InsCertFactory().create(
            receiver=sender,
            routing="/res/data",
            data={"v": "节点不负责转发此指令"},
            status_code=63,
        ...
        raise InsCertException(node=NodeManager().active_node[receiver], ins_cert=ins_cert_send, status_code=61)

    sender_node_model = NodeManager().get_node(sender)
    ...
    node_model = NodeManager().get_node(receiver)
