...
from blocklink.utils.model.ins_record import INS_RECORD
...
async def execute_send_ins(ins: InsOpen | InsCert, is_res=False, timeout=60, websocket=None):
    ...
    # 如果需要跳过 则不进行处理
    if INS_RECORD.is_skip(ins):
        return None

    # 添加指令的BID记录
    INS_RECORD.add_ins(ins)

    #  -------------------------------- 接收者是本节点
    if ins.receiver == NODE_MEAT["bid"] or ins.receiver == NODE_MEAT["bid"][:10] or ins.receiver == "":
        # 本地处理分支
        # 为保证本地同步返回不会抢跑，先注册 future 再进入 handler
        future = None
        if is_res:
            future = RES_FUTURES.add_ins(ins)
